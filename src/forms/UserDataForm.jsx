...
export default function UserData() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-amber-300 p-6">
      <Toaster position="top-right" />

      <form
        onSubmit={handleSubmit}
        className="bg-white shadow-2xl rounded-2xl flex overflow-hidden w-full max-w-5xl"
      >
        {/* Sección izquierda */}
        <div className="bg-amber-500 flex flex-col items-center justify-center p-8 w-1/3">
          <CircleUserRound className="text-white w-24 h-24 mb-4" />
          <h2 className="font-caesar-dressing-regular text-3xl text-white bg-amber-400 px-4 py-2 rounded-xl shadow-md">
            Mi perfil
          </h2>
        </div>

        {/* Sección derecha */}
        <div className="flex-1 p-8 grid grid-cols-2 gap-5 items-start">
          {camposObligatorios.map((campo) => (
            <div key={campo} className="flex flex-col relative">
              ...
              {campo === "grupoSanguineo" ? (
                ...
                // Teléfono de emergencia con ícono
          ...
          <div className="col-span-2 flex justify-center mt-6">
            <ButtonBase
              type="submit"
              label="Aceptar"
              className="bg-amber-500 text-white hover:bg-amber-600 px-8 py-3 rounded-xl shadow-md transition"
            />
          </div>
        </div>
      </form>
