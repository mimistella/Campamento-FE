  ...
  export default function PeriodForm({ periodoId = null, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
      ...
      const inicioPer = new Date(form.fechaInicioPer);
      const finPer = new Date(form.fechaFinPer);
      const inicioInsc = new Date(form.fechaInicioInsc);
      const finInsc = new Date(form.fechaFinInsc);
      const hoy = new Date();

    
      if (inicioPer < hoy) {
        error("La fecha de inicio del período no puede ser anterior a la fecha actual.");
        return;
      }
      if (inicioInsc < hoy) {
        error("La fecha de inicio de inscripción no puede ser anterior a la fecha actual.");
        return;
      }
      if (finInsc >= inicioPer) {
        error("La fecha de fin de inscripción debe ser anterior al inicio del período.");
        return;
      }
      if (inicioPer > finPer) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4 p-2">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className="mt-1 w-full border border-gray-300 rounded-md p-2 focus:ring-amber-400 focus:border-amber-400"
            placeholder="Importe del período"
            min={0}
            step="0.01"
          />
        </div>
