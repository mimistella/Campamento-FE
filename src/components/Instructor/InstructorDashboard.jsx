...
export default function InstructorDashboard() {
  ...
  const user = mydata;
  const { diasCampamento, campamentoEnCurso, loading } = useInstructor();
  ...
  const titulo = campamentoEnCurso
    ? "¡Ya comenzó nuestra aventura!"
    : "¡Falta poco para nuestro próximo campamento!";

  const subtitulo = campamentoEnCurso
    ? "Días que faltan para que termine:"
    : "Días que faltan:";
    ...
    <div className="min-h-screen bg-amber-200 flex items-center justify-center p-6">
      <div className="max-w-2xl w-full bg-white shadow-2xl rounded-3xl p-8 border border-amber-300">
        ...
        <div className="flex flex-col items-center mb-6">
          <p className="text-3xl font-bold text-center text-amber-700 mb-2">
            {titulo}
          </p>
          <p className="text-lg text-amber-800 mb-4">{subtitulo}</p>
          <div className="flex items-baseline">
            <p className="text-6xl font-caesar-dressing-regular text-amber-700 drop-shadow-sm">
              {diasCampamento}
            </p>
            <span className="ml-2 text-lg text-amber-800 font-bold">días</span>
          </div>
        </div>
      </div>
    </div>
