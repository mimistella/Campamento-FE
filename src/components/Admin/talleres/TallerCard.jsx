      ...
      <div className="flex justify-between items-start">
        <div className="flex-1 flex items-center gap-3">
          ...
          {taller.estado === "cancelado" && (
            <span className="text-xs font-semibold bg-orange-100 text-orange-900 px-2 py-1 rounded-full">
              Cancelado
            </span>
          )}
        </div>
      ...
      {isExpanded && (
        <div className="mt-3 space-y-2">
          ...
          <div className="text-sm text-gray-500 space-y-1">
            <p>
              <strong>Fecha:</strong>{" "}
              {new Date(taller.fechaHora).toLocaleString("es-AR")}
            </p>
            <p>
              <strong>Lugar:</strong> {taller.lugar}
            </p>
            <p>
              <strong>Instructor:</strong> {taller.instructor.nombre}{" "}
              {taller.instructor.apellido}
            </p>
            <p>
              <strong>Cupo:</strong> {taller.cupo} personas
            </p>
            <p>
              <strong>Duración:</strong> {taller.duracionMin} minuto
              {taller.duracionMin !== 1 ? "s" : ""}
            </p>
            <p>
              <strong>Período:</strong> {taller.periodo.id}
            </p>
          </div>
        </div>
      )}
